...
/**
 *	\file       htdocs/custom/zonajob/core/modules/commande/doc/pdf_zonajob.modules.php
 *	\ingroup    zonajob
 *	\brief      File of Class to generate PDF orders with template Eratosthene + Signature
 */
...
/**
 *	Class to generate PDF orders with template Eratosthene + Customer Signature
 */
class pdf_zonajob extends ModelePDFCommandes
{
	...
	public function __construct(DoliDB $db)
	{
		...
		$this->db = $db;
		$this->name = "zonajob";
		$this->description = $langs->trans('ZonaJob PDF with Customer Signature');
	...
	public function write_file($object, $outputlangs, $srctemplatepath = '', $hidedetails = 0, $hidedesc = 0, $hideref = 0)
	{
		...
		if (getMultidirOutput($object)) {
			...
			if ($object->specimen) {
				...
				// ZonaJob: Generate unique filename with timestamp when signature exists
				// This ensures each signed PDF has a unique name
				$filename = $objectref;
				dol_include_once('/zonajob/class/zonajobsignature.class.php');
				if (class_exists('ZonaJobSignature')) {
					$sigCheck = new ZonaJobSignature($this->db);
					if ($sigCheck->fetchByOrder($object->id) > 0 && $sigCheck->status == ZonaJobSignature::STATUS_SIGNED) {
						// Add signature date to filename for uniqueness
						$sigDate = $sigCheck->date_signature ? $sigCheck->date_signature : dol_now();
						$filename = $objectref.'_signed_'.dol_print_date($sigDate, 'dayhourlog');
					}
				}
	...
	/**
	 * Draw signature block on the PDF with actual signature image and photos
	 *
	 * @param	TCPDF		$pdf     		Object PDF
	 * @param  Commande	$object			Object order
	 * @param	float		$posy			Position y in PDF
	 * @param  Translate	$outputlangs	Output language
	 * @return	float						New Y position after drawing
	 */
	protected function drawSignatureBlock(&$pdf, $object, $posy, $outputlangs)
	{
		global $conf, $db;

		$pdf->SetFont(pdf_getPDFFont($outputlangs), '', pdf_getPDFFontSize($outputlangs) - 2);

		// Add some space
		$posy += 5;

		// Check if we have already exceeded page height
		if ($posy > $this->page_hauteur - $this->marge_basse - 80) {
			// Need a new page
			$pdf->AddPage();
			$posy = $this->marge_haute;
		}
		...
		$pdf->SetXY($this->marge_gauche, $posy);
		$pdf->MultiCell(0, 5, $outputlangs->trans('CustomerSignature'), 0, 'L');
		$posy += 8;

		// Get signature for this order
		dol_include_once('/zonajob/class/zonajobsignature.class.php');
		$signature = new ZonaJobSignature($db);
		$hasSignature = ($signature->fetchByOrder($object->id) > 0 && $signature->status == ZonaJobSignature::STATUS_SIGNED);

		if ($hasSignature && !empty($signature->signature_file)) {
			// Display actual signature image
			$signatureFile = $conf->commande->dir_output.'/'.$signature->signature_file;

			// Fallback to zonajob signatures folder if not found
			if (!file_exists($signatureFile)) {
				$signatureFile = $conf->zonajob->dir_output.'/signatures/'.$signature->signature_file;
			}
			...
			if (file_exists($signatureFile)) {
				...
				$pdf->SetXY($this->marge_gauche, $posy);
				$pdf->Image($signatureFile, $this->marge_gauche, $posy, $signatureWidth, $signatureHeight, '', '', '', true, 150);

				// Signature info on the right
				$pdf->SetFont(pdf_getPDFFont($outputlangs), '', pdf_getPDFFontSize($outputlangs) - 3);

				$infoX = $this->marge_gauche + $signatureWidth + 10;
				$infoY = $posy;

				$pdf->SetXY($infoX, $infoY);
				$pdf->MultiCell(80, 4, $outputlangs->trans('SignedBy').': '.$signature->signer_name, 0, 'L');
				$infoY += 5;

				$pdf->SetXY($infoX, $infoY);
				$pdf->MultiCell(80, 4, $outputlangs->trans('Date').': '.dol_print_date($signature->date_signature, 'dayhour'), 0, 'L');
				$infoY += 5;

				if (!empty($signature->latitude) && !empty($signature->longitude)) {
					$pdf->SetXY($infoX, $infoY);
					$pdf->MultiCell(80, 4, 'GPS: '.$signature->latitude.', '.$signature->longitude, 0, 'L');
					$infoY += 5;
				}

				if (!empty($signature->ip_address)) {
					$pdf->SetXY($infoX, $infoY);
					$pdf->MultiCell(80, 4, 'IP: '.$signature->ip_address, 0, 'L');
				}

				$posy += $signatureHeight + 5;
			} else {
				// Signature file not found, show info only
				$posy = $this->drawSignatureInfoOnly($pdf, $signature, $posy, $outputlangs);
			}
		} else {
			// No signature - draw placeholder lines
			$posy = $this->drawSignaturePlaceholder($pdf, $posy, $outputlangs);
		}

		// ===== PHOTOS SECTION =====
		$posy = $this->drawPhotosSection($pdf, $object, $posy, $outputlangs);

		// Conformity text
		$pdf->SetFont(pdf_getPDFFont($outputlangs), '', pdf_getPDFFontSize($outputlangs) - 3);
		$pdf->SetXY($this->marge_gauche, $posy + 5);
		$pdf->MultiCell(0, 4, $outputlangs->trans('CustomerAcknowledgesReceiptOfOrder'), 0, 'L');

		return $posy + 15;
	}

	/**
	 * Draw signature placeholder lines when no signature exists
	 *
	 * @param	TCPDF		$pdf     		Object PDF
	 * @param	float		$posy			Position y in PDF
	 * @param  Translate	$outputlangs	Output language
	 ...
	 */
	protected function drawSignaturePlaceholder(&$pdf, $posy, $outputlangs)
	{
		$linewidth = 50;
		$pdf->SetFont(pdf_getPDFFont($outputlangs), '', pdf_getPDFFontSize($outputlangs) - 2);
		...
		$pdf->Line($this->marge_gauche, $posy + 30, $this->marge_gauche + $linewidth, $posy + 30);

		// Signature label
		$pdf->SetXY($this->marge_gauche, $posy + 32);
		$pdf->MultiCell($linewidth, 5, $outputlangs->trans('SignatureOfCustomer'), 0, 'C');

		// Date line
		$pdf->SetXY($this->marge_gauche + $linewidth + 20, $posy);
		$pdf->Line($this->marge_gauche + $linewidth + 20, $posy + 30, $this->marge_gauche + $linewidth + 20 + $linewidth, $posy + 30);

		// Date label
		$pdf->SetXY($this->marge_gauche + $linewidth + 20, $posy + 32);
		$pdf->MultiCell($linewidth, 5, $outputlangs->trans('Date'), 0, 'C');

		return $posy + 40;
	}

	/**
	 * Draw signature info when file is missing
	 *
	 * @param	TCPDF				$pdf     		Object PDF
	 * @param	ZonaJobSignature	$signature		Signature object
	 * @param	float				$posy			Position y in PDF
	 * @param  Translate			$outputlangs	Output language
	 * @return	float								New Y position
	 */
	protected function drawSignatureInfoOnly(&$pdf, $signature, $posy, $outputlangs)
	{
		$pdf->SetFont(pdf_getPDFFont($outputlangs), '', pdf_getPDFFontSize($outputlangs) - 2);

		$pdf->SetXY($this->marge_gauche, $posy);
		$pdf->MultiCell(0, 5, $outputlangs->trans('SignedBy').': '.$signature->signer_name, 0, 'L');
		$posy += 6;

		$pdf->SetXY($this->marge_gauche, $posy);
		$pdf->MultiCell(0, 5, $outputlangs->trans('Date').': '.dol_print_date($signature->date_signature, 'dayhour'), 0, 'L');
		$posy += 6;

		if (!empty($signature->latitude) && !empty($signature->longitude)) {
			$pdf->SetXY($this->marge_gauche, $posy);
			$pdf->MultiCell(0, 5, 'GPS: '.$signature->latitude.', '.$signature->longitude, 0, 'L');
			$posy += 6;
		}
	 ...
	 * @return	float						New Y position after drawing
	 */
	protected function drawPhotosSection(&$pdf, $object, $posy, $outputlangs)
	{
		global $conf, $db;
		...
		$photos = $photoObj->getPhotosForOrder($object->id);

		if (empty($photos)) {
			return $posy;
		}

		// Add space before photos section
		$posy += 10;

		// Check if we need a new page
		if ($posy > $this->page_hauteur - $this->marge_basse - 60) {
			$pdf->AddPage();
			$posy = $this->marge_haute;
		}
		...
		$pdf->SetFont(pdf_getPDFFont($outputlangs), 'B', pdf_getPDFFontSize($outputlangs));
		$pdf->SetXY($this->marge_gauche, $posy);
		$pdf->MultiCell(0, 5, $outputlangs->trans('AttachedPhotos').' ('.count($photos).')', 0, 'L');
		$posy += 8;
		...
		foreach ($photos as $photo) {
			...
			if ($currentCol >= $photosPerRow) {
				$currentCol = 0;
				$startY += $photoHeight + 15;

				if ($startY > $this->page_hauteur - $this->marge_basse - $photoHeight - 15) {
					$pdf->AddPage();
			...
			// Calculate position
			$x = $startX + ($currentCol * ($photoWidth + $margin));
			$y = $startY;

			// Get photo file path
			$photoFile = '';
			if (!empty($photo->filepath)) {
				// Try ECM folder first (documents/commande/REF/)
				$photoFile = $conf->commande->dir_output.'/'.$object->ref.'/'.$photo->filepath;
				if (!file_exists($photoFile)) {
					$photoFile = $conf->commande->dir_output.'/'.$object->ref.'/'.$photo->filename;
				}
				if (!file_exists($photoFile)) {
					// Try zonajob photos folder
					$photoFile = $conf->zonajob->dir_output.'/photos/'.$photo->filepath;
				}
				if (!file_exists($photoFile)) {
					$photoFile = $conf->zonajob->dir_output.'/photos/'.$photo->filename;
				}
			}

			// Draw photo if file exists
			if (!empty($photoFile) && file_exists($photoFile)) {
				// Draw border
				$pdf->SetDrawColor(200, 200, 200);
				$pdf->SetLineWidth(0.2);
				$pdf->Rect($x - 1, $y - 1, $photoWidth + 2, $photoHeight + 2);

				// Draw photo
				$pdf->Image($photoFile, $x, $y, $photoWidth, $photoHeight, '', '', '', true, 150);

				// Draw caption below photo
				$caption = !empty($photo->description) ? $photo->description : $photo->photo_type;
				$pdf->SetXY($x, $y + $photoHeight + 1);
				$pdf->MultiCell($photoWidth, 3, dol_trunc($caption, 20), 0, 'C');

				// Add date if available
				if (!empty($photo->date_creation)) {
					$pdf->SetXY($x, $y + $photoHeight + 5);
					$pdf->SetFont(pdf_getPDFFont($outputlangs), '', pdf_getPDFFontSize($outputlangs) - 4);
					$pdf->MultiCell($photoWidth, 3, dol_print_date($photo->date_creation, 'day'), 0, 'C');
					$pdf->SetFont(pdf_getPDFFont($outputlangs), '', pdf_getPDFFontSize($outputlangs) - 3);
				}
			} else {
				// Photo file not found - draw placeholder
				$pdf->SetDrawColor(200, 200, 200);
				$pdf->SetFillColor(245, 245, 245);
				$pdf->Rect($x, $y, $photoWidth, $photoHeight, 'DF');
				$pdf->SetXY($x, $y + ($photoHeight / 2) - 3);
				$pdf->MultiCell($photoWidth, 4, $outputlangs->trans('PhotoNotFound'), 0, 'C');
			}
